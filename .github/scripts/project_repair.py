...
def _discover_activity_from_sources(project_root: str):
    """Détecte prudemment une activité Android quand aucun manifeste n’est disponible."""
    java_pattern = re.compile(r"class\s+(\w+)\s+extends\s+(?:[\w.]*)(?:Activity|AppCompatActivity|ComponentActivity)")
    kotlin_pattern = re.compile(r"class\s+(\w+)\s*\([^)]*\)\s*:\s*(?:[\w.]*)(?:Activity|AppCompatActivity|ComponentActivity)")
    for abs_path, rel_path in walk_files(project_root):
        if not abs_path.endswith((".java", ".kt")):
            continue
        content = read_text(abs_path) or ""
        package_match = re.search(r"^\s*package\s+([\w.]+)", content, re.MULTILINE)
        class_match = (java_pattern if abs_path.endswith(".java") else kotlin_pattern).search(content)
        if package_match and class_match:
            return {"package": package_match.group(1), "activity": "." + class_match.group(1), "source": rel_path}
    return None
...
def _generate_manifest(project_root: str, analysis: dict, report: dict):
    ...
    act_info = launcher_acts[0] if launcher_acts else _discover_activity_from_sources(project_root)
...
"""
    ...
    reason = "manifest absent — généré"
    if act_info and act_info.get("source"):
        reason += f" avec activité détectée dans {act_info['source']}"
    log_action(report, "created", file=dst_rel, reason=reason)
...
def repair(project_root: str) -> dict:
    ...
    # 4. Vérification minimale. En mode secours, l’outil ne prétend pas inventer
    # la logique métier absente : il génère un APK installable qui explique
    # clairement pourquoi la reconstruction originale n’était pas possible.
    has_sources = bool(analysis["kotlinFiles"] or analysis["javaFiles"])
    if not has_sources:
        allow_fallback = os.environ.get("ANDROID_BUILDER_ALLOW_FALLBACK", "1") == "1"
        ...
        from bootstrap_app import build_rescue, source_summary
        original_summary = source_summary(Path(project_root))
        print("[FALLBACK] Aucun code source exploitable — génération d’un APK Android de secours.")
        fallback = build_rescue(
            Path(project_root),
            original_summary,
            "Le ZIP importé ne contient aucun fichier Kotlin ou Java Android exploitable."
        )
        log_action(report, "created", file="projet de secours", reason="sources Android absentes")
        report["fallback"] = fallback
        report["post_repair_score"] = {"pct": 100, "total": 1, "max": 1, "mode": "fallback-rescue"}
        save_report(report_path, report)
        return report

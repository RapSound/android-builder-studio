...
// Éléments bloquants : sans eux, impossible de compiler quoi que ce soit,
// et on ne peut pas les générer nous-mêmes sans connaître le code du projet.
const REQUIRED_HARD = [
  { key: "manifest", label: "AndroidManifest.xml", test: (p, root) => p === root + "AndroidManifest.xml" || p === root + "app/src/main/AndroidManifest.xml" },
  { key: "buildGradle", label: "build.gradle ou build.gradle.kts (racine)", test: (p, root) => p === root + "build.gradle" || p === root + "build.gradle.kts" },
  { key: "settingsGradle", label: "settings.gradle ou settings.gradle.kts", test: (p, root) => p === root + "settings.gradle" || p === root + "settings.gradle.kts" },
  { key: "appDir", label: "dossier app/", test: (p, root) => p.startsWith(root + "app/") },
];

// Éléments optionnels : leur absence n'empêche pas la compilation
// (le workflow les régénère ou s'en passe), mais on prévient l'utilisateur
// et on peut les ajouter automatiquement au ZIP avant envoi.
const OPTIONAL_AUTOFIX = [
  {
    key: "gradleProps",
    label: "gradle.properties",
    test: (p, root) => p === root + "gradle.properties",
    fix: (zip, root) => zip.file(root + "gradle.properties", DEFAULT_GRADLE_PROPERTIES),
  },
  {
    key: "wrapperProps",
    label: "gradle/wrapper/gradle-wrapper.properties",
    test: (p, root) => p === root + "gradle/wrapper/gradle-wrapper.properties",
    fix: (zip, root) => zip.file(root + "gradle/wrapper/gradle-wrapper.properties", DEFAULT_WRAPPER_PROPERTIES),
  },
  {
    key: "gitignore",
    label: ".gitignore",
    test: (p, root) => p === root + ".gitignore",
    fix: (zip, root) => zip.file(root + ".gitignore", DEFAULT_GITIGNORE),
  },
];
// Remarque : le wrapper gradlew/gradle-wrapper.jar n'est plus exigé ici —
// le workflow le régénère automatiquement côté serveur s'il est absent ou invalide.
...
async function handleZip(file) {
  if (!file.name.toLowerCase().endsWith(".zip")) {
    showVerify({ blocking: ["Le fichier doit être une archive .zip"], optional: [] });
    return;
  }
  ...
  try {
    ...
    showVerify({ blocking: ["ZIP invalide ou corrompu : impossible de le lire."], optional: [] });
    return;
  }

  const paths = Object.keys(zip.files);
  const rootPrefix = detectRootPrefix(paths);
  const result = analyzeProject(paths, rootPrefix);
...
function analyzeProject(paths, rootPrefix) {
  const blocking = [];
  const optional = [];
  const fixable = [];

  REQUIRED_HARD.forEach((rule) => {
    const found = paths.some((p) => rule.test(p, rootPrefix));
    if (!found) blocking.push(rule.label);
  });

  OPTIONAL_AUTOFIX.forEach((rule) => {
    const found = paths.some((p) => rule.test(p, rootPrefix));
    if (!found) {
      optional.push(rule.label);
      fixable.push(rule.key);
    }
  });

  const hasWrapper = paths.some((p) => p === rootPrefix + "gradlew") && paths.some((p) => p === rootPrefix + "gradle/wrapper/gradle-wrapper.jar");
  if (!hasWrapper) {
    optional.push("gradlew / gradle-wrapper.jar (sera régénéré automatiquement pendant la compilation)");
  }

  return { blocking, optional, fixable };
}
...
async function autoCompleteZip() {
  ...
  OPTIONAL_AUTOFIX.forEach((rule) => {
    if (state.lastValidation.fixable.includes(rule.key)) {
      rule.fix(zip, root);
      fixed.push(rule.label);
    }
  });
  ...
  const paths = Object.keys(zip.files);
  const result = analyzeProject(paths, root);
  state.lastValidation = result;
  showVerify(result);

  toast("Ajouté automatiquement : " + fixed.join(", "), "success");
}
...
function showVerify(result) {
  const { blocking, optional } = result;
  ...
  if (blocking.length > 0) {
    ...
    el.innerHTML =
      "✕ Projet invalide, éléments manquants requis :" +
      "<ul>" + blocking.map((m) => `<li>${escapeHtml(m)}</li>`).join("") + "</ul>" +
      "<p class=\"hint\">Ces éléments doivent venir du projet lui-même : ajoute-les dans ton IDE puis réexporte le ZIP.</p>";
    return;
  }
  ...
  if (optional.length > 0) {
    html +=
      "<p class=\"hint\" style=\"margin-top:10px;\">Éléments recommandés absents (non bloquants) :</p>" +
